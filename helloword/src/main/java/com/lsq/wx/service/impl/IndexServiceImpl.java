package com.lsq.wx.service.impl;
...
import com.lsq.wx.util.HttpUtil;
import com.lsq.wx.util.PayUtils;
import com.lsq.wx.vo.AuthToken;
import com.lsq.wx.vo.PaySendData;
@Service
public class IndexServiceImpl implements IndexService{

	@Override
	public String pay(AuthToken authToken, String remoteAddr) {
		Map<String, String> resultMap = null;
		// 统一下单返回的预支付id
		String prepayId = null;
		PaySendData paySendData = new PaySendData();
		// 构建微信支付请求参数集合
		paySendData.setAppId(StaticVar.APP_ID);
		paySendData.setMch_id(StaticVar.MCH_ID);
		paySendData.setNotify_url(StaticVar.NOTIFY_URL);
		paySendData.setTrade_type(StaticVar.TRADE_TYPE_JSAPI);
		paySendData.setDevice_info(StaticVar.WEB);
		...
		try {
			// 发送请求
			CloseableHttpResponse response = HttpUtil.Post(StaticVar.UNIFIED_ORDER_URL, reqXml, false);
			try {
		...
		if (StaticVar.RETURN_SUCCESS.equals(return_code) && StaticVar.RETURN_SUCCESS.equals(result_code)) {
			// return_code=通信标识
			// result_code=交易标识
	...
	public String refund() {
		...
		paySendData.setOp_user_id(StaticVar.MCH_ID);
		// 将参数拼成map,生产签名
		paySendData.setSign(PayUtils.getSign(buildParamMap(paySendData)));
		// 将请求参数对象转换成xml
		String reqXml = PayUtils.sendDataToXml(paySendData);
		try {
			// 发送请求
			CloseableHttpResponse response = HttpUtil.Post(StaticVar.UNIFIED_ORDER_URL, reqXml, true);
			try {
				// 关闭流
				EntityUtils.consume(response.getEntity());
			} finally {
